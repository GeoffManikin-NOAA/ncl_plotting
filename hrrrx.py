...
#for j in range(1):
   ...
   for ax in axes:
      ...
      if k == 0:
         model_str = 'HRRRX'
         try:
             fil = DATA_DIR+'/hrrrx.'+cycle[0:8]+'/'+str.lower(model_str)+'.t'+ \
      ...
      if model_cycle is None:
         fil = DATA_DIR+'/hrrrx.'+cycle[0:8]+'/hrrrx.t'+ \
               cycle[8:10]+'z.wrfsfcf'+str(fhrs[j]).zfill(2)+'.grib2'
      ...
      elif str.upper(model_str) == 'NAM3' or str.upper(model_str) == 'HRRRX':
         HLwindow = 300
         if domain in large_domains:
            skip = 50
         elif domain in subregional_domains:
      ...
      # Simulated reflectivity
         ...
         if k != 5:
            try:
                ...
                refc = grbs.select(stepType='instant',parameterCategory=16,parameterName="196",typeOfLevel='atmosphere',level=0)[0].values
