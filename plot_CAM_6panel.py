...
elif cycle == '2022050300':
   fhrs = np.arange(39,49,3)
elif cycle == '2022050312':
   fhrs = np.arange(27,40,3)
elif cycle == '2022050400':
   fhrs = np.arange(15,28,3)
elif cycle == '2022050412':
   fhrs = np.arange(3,16,3)
elif cycle == '2022050413':
   fhrs = np.arange(5,12,3)
elif cycle == '2022050414':
   fhrs = np.arange(4,11,3) 
elif cycle == '2022050415':
   fhrs = np.arange(6,10,1)
elif cycle == '2022050416':
   fhrs = np.arange(2,9,3)
elif cycle == '2022050417':
   fhrs = np.arange(1,8,3)
elif cycle == '2022050418':
   fhrs = np.arange(3,11,3)
elif cycle == '2022050419':
   fhrs = np.arange(5,10,2)
elif cycle == '2022042912':
   fhrs = np.arange(3,16,3)
elif cycle == '2022042913':
   fhrs = np.arange(8,15,3)
elif cycle == '2022042914':
   fhrs = np.arange(7,14,3)
elif cycle == '2022042915':
   fhrs = np.arange(6,13,3)
elif cycle == '2022042916':
   fhrs = np.arange(5,10,3)
elif cycle == '2022042917':
   fhrs = np.arange(4,10,3)
elif cycle == '2022042918':
   fhrs = np.arange(3,9,1)
elif cycle == '2022042919':
   fhrs = np.arange(2,8,1)
elif cycle == '2022042920':
   fhrs = np.arange(1,7,1)
elif cycle == '2022042921':
   fhrs = np.arange(1,6,1)
elif cycle == '2022061300':
   fhrs = np.arange(20,40,2)
elif cycle == '2022061312':
   fhrs = np.arange(8,28,2)
elif cycle == '2022061400':
   fhrs = np.arange(0,16,2)


# set forecast range here.   first value is first forecast hour to plot
# second number is final forecast hour to plot + 1
# third number is increment
...
#for j in range(1):
   ...
   for ax in axes:
      ...
      if domain == 'conus':
          ...
          urcrnrlat=45.
          urcrnrlon=-68.
      elif domain == 'may22svr2':
          llcrnrlat=38.
          llcrnrlon=-104.
          urcrnrlat=49.
          urcrnrlon=-88.
      elif domain == 'may22svr':
          llcrnrlat=30.
          llcrnrlon=-104.
          urcrnrlat=38.
          urcrnrlon=-92.
      ...
      m = Basemap(llcrnrlon=llcrnrlon,llcrnrlat=llcrnrlat,urcrnrlon=urcrnrlon,urcrnrlat=urcrnrlat,\
                  ...
                  lat_1=32.,lat_2=46.,lon_0=-82.,area_thresh=1000.,ax=ax)
      ...
      if k == 0:
         model_str = 'HRRR'
         try:
             fil = DATA_DIR+'/hrrr.'+cycle[0:8]+'/'+str.lower(model_str)+'.t'+ \
                   cycle[8:10]+'z.wrfsfcf'+str(fhrs[j]).zfill(2)+'.grib2'
         ...
         model_str = 'HiResWARW'
         try:
             fil = DATA_DIR+'/hiresw.'+cycle[0:8]+'/'+str.lower(model_str[0:6])+'.t'+ \
      ...
      # Obs panel - MRMS for REFC
         ...
         fil = DATA_DIR+'/mrms/MergedReflectivityQCComposite_00.50_'+valid[0:8]+'-'+valid[8:10]+'0000_g227.nc'
      ...
      # Obs panel - RAP for CAPE
         ...
         try:
             ...
             fil = DATA_DIR+'/rap.'+valid[0:8]+'/'+str.lower(model_str[0:3])+'.t'+ \
                   valid[8:10]+'z.awp130pgrbf00.grib2.new'
             grbs = pygrib.open(fil)
      ...
      if model_cycle is None:
         fil = DATA_DIR+'/hrrr.'+cycle[0:8]+'/hrrr.t'+ \
               cycle[8:10]+'z.wrfsfcf'+str(fhrs[j]).zfill(2)+'.grib2'
      ...
      if field[0:2] == 't2':
         ...
         titlestr = date_str.strftime('%HZ %m/%d ')+field_title+' Forecasts and Analysis '+valid_list[j].strftime('Valid at %HZ %d %b %Y')+' (F'+str(fhrs[j]).zfill(2)+')'
      ...
      # CAPE
         ...
         if field[0:2] == 'sb':
            ...
            mucin  = grbs.select(name='Convective inhibition',typeOfLevel='pressureFromGroundLayer',topLevel=18000)[0].values               
...
#           plt.clabel(contours,cint,colors='k',inline=1,fmt='%.0f',fontsize=9)
      ...
      # Simulated reflectivity
         ...
         titlestr = date_str.strftime('%HZ %m/%d ')+field_title+' Forecasts and Observations '+valid_list[j].strftime('Valid at %HZ %d %b %Y')+' (F'+str(fhrs[j]).zfill(2)+')'
      ...
      # Hourly-max 10-m wind
         ...
         titlestr = date_str.strftime('%HZ %m/%d ')+field_title+' Forecasts '+valid_list[j].strftime('Valid at %HZ %d %b %Y')+' (F'+str(fhrs[j]).zfill(2)+')'
